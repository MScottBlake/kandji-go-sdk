# Go API client for kandji_go_sdk
...
For more information, please visit [https://github.com/MScottBlake/kandji-openapi-spec](https://github.com/MScottBlake/kandji-openapi-spec)
...
```go
import kandji_go_sdk "github.com/MScottBlake/kandji-go-sdk"
```
...
For using other server than the one defined on index 0 set context value `kandji_go_sdk.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), kandji_go_sdk.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `kandji_go_sdk.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), kandji_go_sdk.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `kandji_go_sdk.ContextOperationServerIndices` and `kandji_go_sdk.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), kandji_go_sdk.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), kandji_go_sdk.ContextOperationServerVariables, map[string]map[string]string{
...
```go
auth := context.WithValue(context.Background(), kandji_go_sdk.ContextAccessToken, "BEARER_TOKEN_STRING")
